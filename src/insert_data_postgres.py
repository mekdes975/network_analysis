...
def main():
    ...
    try:
        ...
        insert_data_into_table(connection, cursor, "users", pd.read_csv(users_data_file).to_dict('records'))

        # Insert data into the 'channels' table
        channels_data_file = "../data/channels.csv"
        insert_data_into_table(connection, cursor, "channels", pd.read_csv(channels_data_file).to_dict('records'))

        # Insert data into the 'channel_members' table
        channel_members_data_file = "../data/channel_members.csv"
        insert_data_into_table(connection, cursor, "channel_members", pd.read_csv(channel_members_data_file).to_dict('records'))
